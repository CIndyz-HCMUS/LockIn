...
export function QuickAddSleepModal(props: {
  open: boolean;
  dateKey: string;
  onClose: () => void;
  onSaved: () => void;
  ...
  const canSave = useMemo(() => Number.isFinite(minutes) && minutes > 0, [minutes]);
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      // backend: POST /logs/sleep
      await postJson("/logs/sleep", { date: dateKey, minutes });
      onSaved();
    } finally {
      setSaving(false);
    }
    ...
    <div style={styles.overlay} onMouseDown={onClose}>
      <div style={styles.card} onMouseDown={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <div>
            <div style={{ fontWeight: 900, fontSize: 16 }}>Quick Add • Sleep</div>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ fontWeight: 800, marginBottom: 6 }}>Sleep (minutes)</div>
          <input
            type="number"
            value={minutes}
            min={1}
