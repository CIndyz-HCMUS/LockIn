...
export function QuickAddWaterModal(props: {
  open: boolean;
  dateKey: string;
  onClose: () => void;
  onSaved: () => void;
  ...
  const canSave = useMemo(() => Number.isFinite(ml) && ml > 0, [ml]);
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      // backend: POST /logs/water
      await postJson("/logs/water", { date: dateKey, ml });
      onSaved();
    } finally {
      setSaving(false);
    }
    ...
    <div style={styles.overlay} onMouseDown={onClose}>
      <div style={styles.card} onMouseDown={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <div>
            <div style={{ fontWeight: 900, fontSize: 16 }}>Quick Add • Water</div>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ fontWeight: 800, marginBottom: 6 }}>Amount (ml)</div>
          <input
            type="number"
            value={ml}
            min={1}
